...
public class RewardByDiscountServiceTest {
	...
	void setUp() {
		...
		order = Arrays.asList(bigDecaf, bigLatte, bigTea, espresso);
...
//		collector.checkThat(info.getDiscount() > 10, is(true));
		collector.checkThat(100L, equalTo(info.getPointsRedeemed()));
//		collector.checkThat(1000L, equalTo(info.getPointsRedeemed()));
